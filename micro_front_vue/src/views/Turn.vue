<template>
  <div class="container">
    ...
    <div class="main">
      ...
      <div class="bg-p"></div>
      <div class="content">
        <div class="count">剩余积分数： {{ score}}</div>
      </div>
    </div>
...
export default {
 name:'Turn',
  ...
  created() {
    this.cost= parseInt(this.$route.query.cost)
    // 初始化一些值
    this.angleList = [],
    // 是否正在旋转
  ...
  computed: {
    ...
    toastTitle () {
      return this.prize&&this.prize.name!="谢谢惠顾"
        ? "恭喜您，获得" +
            this.prize.name
        : "未中奖";
    },
  ...
  methods: {
	async getScore(){
	  ...
	  if (!email) {
	   ...
	   const { data: res } = await this.$http.get('user/score')
	   this.score=res.data
	},
    async initPrizeList() {
      ...
      // 这里可以发起请求，从服务端获取奖品列表
      // 这里使用模拟数据
	 const { data: res } = await this.$http.get('activity/prizes' ,{
       params:{
          id:this.$route.params.id,
           }
      })
    ...
    async beginRotate() {
      console.log(this.id);
      // 添加次数校验
      if(this.score -this.cost<=0) return
	  const form_data=new FormData()
	  form_data.append("id",this.$route.params.id)
	  const { data: res } = await this.$http.post('choose/do' ,form_data)
	  if (res.code!=200) {
		return alert(res.msg)
	  }
	   const {data:res2}=await this.$http.get('choose/result')
       if (res2.code!=200) {
	  ...
	  for (let index = 0; index < this.prizeList.length; index++) {
		if (this.prizeList[index].name===res2.data) {
			this.index=index
			break
		}		
	  }
